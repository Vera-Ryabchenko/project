...
<?
    ...
    if ($roletype->id_role == 2){
        $selectComments = $mysqli->query("SELECT * FROM `feedback` WHERE `idusers` = '$id' AND `messagestatus` = 'решена' ORDER BY timemessage DESC");
    }
    elseif ($roletype->id_role == 1) {
        $selectComments = $mysqli->query("SELECT * FROM `feedback` WHERE `messagestatus` = 'решена' ORDER BY timemessage DESC");
    }
