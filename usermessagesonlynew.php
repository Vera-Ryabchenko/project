...
<?
    ...
    if ($roletype->id_role == 2){
        $selectComments = $mysqli->query("SELECT * FROM `feedback` WHERE `idusers` = '$id' AND `messagestatus` = 'новая' ORDER BY timemessage DESC");
    }
    elseif ($roletype->id_role == 1) {
        $selectComments = $mysqli->query("SELECT * FROM `feedback` WHERE `messagestatus` = 'новая' ORDER BY timemessage DESC");
    }
